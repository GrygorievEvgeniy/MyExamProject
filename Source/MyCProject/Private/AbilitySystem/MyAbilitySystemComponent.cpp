...
UMyAbilitySystemComponent::UMyAbilitySystemComponent(): InputComponent(nullptr)
{
}

void UMyAbilitySystemComponent::SetInputBinding(UInputAction* InputAction, const FGameplayAbilitySpecHandle AbilityHandle)
{
...
void UMyAbilitySystemComponent::ClearInputBinding(FGameplayAbilitySpecHandle AbilityHandle)
{
	using namespace EnhancedInputAbilitySystem_Impl;
...
void UMyAbilitySystemComponent::ClearAbilityBindings(UInputAction* InputAction)
{
	RemoveEntry(InputAction);
}

void UMyAbilitySystemComponent::OnAbilityInputPressed(UInputAction* InputAction)
{
	using namespace EnhancedInputAbilitySystem_Impl;
...
void UMyAbilitySystemComponent::OnAbilityInputReleased(UInputAction* InputAction)
{
	using namespace EnhancedInputAbilitySystem_Impl;
...
void UMyAbilitySystemComponent::RemoveEntry(UInputAction* InputAction)
{
...
FGameplayAbilitySpec* UMyAbilitySystemComponent::FindAbilitySpec(FGameplayAbilitySpecHandle Handle)
{
...
void UMyAbilitySystemComponent::TryBindAbilityInput(UInputAction* InputAction, FAbilityInputBinding& AbilityInputBinding)
{
	if (InputComponent)
	{
		// Pressed event
		...
		{
			AbilityInputBinding.OnPressedHandle = InputComponent->BindAction(InputAction, ETriggerEvent::Started, this, &UMyAbilitySystemComponent::OnAbilityInputPressed, InputAction).GetHandle();
		}

		// Released event
		if (AbilityInputBinding.OnReleasedHandle == 0)
		{
			AbilityInputBinding.OnReleasedHandle = InputComponent->BindAction(InputAction, ETriggerEvent::Completed, this, &UMyAbilitySystemComponent::OnAbilityInputReleased, InputAction).GetHandle();
		}
	}
}
